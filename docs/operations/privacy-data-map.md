...
| PDM-EXPORT-001            | 用户                    | source revision vector/fingerprint、24 小时 manifest、下载状态；导出正文仅 T0            | 用户权利数据          | 高        | T0/T2 | 向用户提供自身数据副本且不新增正文持久副本                | BASIS-EXPLICIT            |
| PDM-RIGHTS-TASK-001       | 用户                    | DataTask、DeletionGuard、step checkpoint、failure scope、单 task status-token hash       | 受限权利流程          | 高        | T2    | 同步阻断、执行并在普通 session 吊销后展示同一删除任务状态 | BASIS-EXPLICIT / SECURITY |
...
| PDM-EXPORT-001            | 数据管理页                         | DataRightsSummary、DataTaskView、export command、owner download | Export DataTask / Manifest             | `DataTask(kind=EXPORT)` + `ExportManifest`；正文不落库/对象                                                                                                                         | READY/EXPIRED/INVALIDATED、download ref 和真实到期时间                                  |
| PDM-RIGHTS-TASK-001       | 删除确认/任务页                    | DataTaskView、prepare/confirm/task/status queries               | Data Task / Guard / Status Grant       | `DataTask`；`DeletionGuard`；`DeletionStepCheckpoint`；`DeletionStatusGrant(tokenHash)`                                                                                             | scope、阶段、online erased、backup deadline；明文 token 仅 confirm 响应一次；无被删正文 |
...
| 小程序本地缓存            | 仅经规格批准的非敏感短期 View；ACCOUNT 删除流程可保存一个最长 7 天的 status token                     | 自由文本、事项、note、Safety、普通 session token 副本或 status token 的日志/analytics/分享 | status token 只读一个 task，完成/失效后删除；旧 epoch 不得离线恢复普通页 |
| PostgreSQL `app_*`        | T1 权威账户、日事实、互动、关系、事项、周数据                                                         | raw provider response、普通日志、generic memory text                                       | retention 元数据、scope guard、硬删除                                    |
| PostgreSQL `restricted_*` | Safety、DataTask、guard、回执、restore deny、legal hold、受限审计、export manifest、status-token hash | 导出正文、明文 status token、普通产品查询和运营浏览                                        | 专用角色、最小字段、独立 TTL；manifest 不含 body/object key              |
...
| 对象存储 / CDN            | 短期分享图、加密备份；C-014 v1 导出不创建对象                                                         | 默认长期用户内容仓库、v1 导出正文                                                          | 分享 7 天、在线删除 72h、备份 35 天；未来持久导出对象需新 ADR            |
...
| PDM-EXPORT-001                            | READY 后 24h；正文只存在于单次下载内存/响应                | download ref 先失效；无对象/DB body 可删                                  | EXPORT/ACCOUNT；manifest 与 task 元数据 terminal 后 30 天                  |
...
| PDM-RIGHTS-TASK-001                       | 直到完成或失败解决；ACCOUNT status grant 最长 7 天且不刷新 | SUCCEEDED 首次状态读取或到期清理 token hash；task/export 元数据按策略清理 | grant 不能读取其它 task/普通 API；guard 失败时仍保持生效                   |
...
| 导出                   | `GET /data-rights/summary` → `POST /data-rights/export` → owner-authenticated download           | manifest 24h、正文零持久化、2 MiB 整包上限；包含自身 active product 数据和第 12.1 节允许的受限摘要；不包含 secret、Prompt、其它用户数据或内部安全字段 |
