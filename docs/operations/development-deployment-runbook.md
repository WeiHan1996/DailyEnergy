...
控制器固定执行 18 个阶段：preflight、digest pull、stateful readiness、关闭 loopback TLS 进入 DEV 维护、drain workers、migration 与 drift verify、Interactive、Background、API、Admin、Restricted、恢复 TLS、health、object smoke、Safety smoke、owner smoke、deletion smoke、退出维护。DEV_LITE 在 pull/migration 前停止 application/transient workload但保留 PostgreSQL、Redis 与 dependency stub 供失败恢复 probe 使用；每个临时 profile 启动前先停止其它临时 profile，所有 one-shot 使用确定性容器名防止中断重试并发。one-shot 命令必须同时激活 `dev-lite-core` 与 `dev-lite-one-shot` 以满足 Compose profile dependency model，但固定使用 `run --no-deps`；Admin、Interactive、Background 与 Restricted convergence 也必须同时激活 `dev-lite-core` 与各自 profile，并固定 `up --no-deps`。激活 dependency model 不得隐式启动第二组 core 或其它 transient workload。每阶段 receipt 前检查期望 service set、health、OOM、restart、20 GiB 磁盘余量和受保护端口。DEV_LITE 的 Compose 闭集全部是 exact-digest reference，pull 使用 `--policy missing`：缺失内容仍必须从 registry 拉取，已经离线导入且可由完整 `tag@sha256` 解析的内容可以复用；历史 STANDARD DEV 继续使用 `--policy always`。Admin 窗口会临时启动 TLS、验证 `8444/login` 后关闭 TLS；object smoke 是本地 one-shot，标准 DEV 才使用 COS。任一阶段失败都不写 Accepted release state。
