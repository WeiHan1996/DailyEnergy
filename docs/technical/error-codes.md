...
| HTTP | 典型 code                                                                                                   |
| ---- | ----------------------------------------------------------------------------------------------------------- |
| 400  | `VALIDATION_FAILED`、`UNSUPPORTED_LOCALE`、`AUTH_WECHAT_CODE_INVALID`                                       |
...
| 429  | `RATE_LIMITED`                                                                                              |
| 503  | `UPSTREAM_TRANSIENT`、`GENERATION_PENDING`、`DEPENDENCY_UNAVAILABLE`、`SAFETY_INDETERMINATE`                |
...
| `EXPORT_ARTIFACT_EXPIRED`         | GUARD      | no        | 导出 READY 访问期已超过 24 小时，必须创建新 intent         |
| `EXPORT_SOURCE_CHANGED`           | CONFLICT   | no        | 导出源 revision 已变化，旧 manifest 已失效                 |
| `EXPORT_TOO_LARGE`                | TERMINAL   | no        | 未压缩 UTF-8 超过 2 MiB，整份拒绝且不截断                  |
| `DELETION_STATUS_GRANT_INVALID`   | AUTH       | no        | status token/hash/task 不匹配、已终止或到期                |
