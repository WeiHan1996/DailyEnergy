...
- **最后更新**：2026-08-28（C-014 按 ADR-0008 增补 export manifest 与 deletion-status grant）
...
- **上游权威**：[领域模型](../data/domain-model.md)、[ADR-0005](../decisions/ADR-0005-data-retention-and-deletion.md)、[ADR-0008](../decisions/ADR-0008-data-rights-delivery-and-status.md)、[数据权利传输契约修订](./data-rights-contract-amendment.md)、[共享 Schema](../../packages/shared-schemas/README.md)
...
| SQL-011 | DataTask 时间单调且成功语义按 kind 分离：DELETE 需要 onlineErasedAt/backup deadline；EXPORT 需要终态 manifest 且不得伪造 erasure 时间 |
...
同一事务校验确认/expected revision，创建或读取 active DataTask，递增 scope DeletionGuard，并写 guardedAt/outbox。ACCOUNT confirm 同事务创建最长 7 天、hash-only 的单 task status grant 并吊销普通 session。事务提交即 semantic blocked；后续 worker 失败也不释放 guard。
...
- C-014 `DataRightsRetentionDue` 从 PostgreSQL 的 manifest/grant/task deadline 重建：READY 24 小时转 EXPIRED、status-token hash 在成功终态读取或 7 天到期清理、Export manifest/task 元数据 terminal 后 30 天清理；Redis 不是期限事实源。

### 11.3 Export manifest 与删除状态 grant

- `restricted_export_manifest` 每个 Export DataTask 最多一行，只含 source revision vector、不可逆 SHA-256 fingerprint、schema/policy version、opaque download ref、READY/expiry/invalidated 时间；禁止 body、object key、用户正文和通用 cache；
- download 在 account guard fence 内先比较当前 vector，变化则原子 `INVALIDATED`，到期则 `EXPIRED`；只有 active owner session 能取得单次内存投影；
- `restricted_deletion_status_grant` 每个 ACCOUNT DataTask 最多一行，只存 32-byte token hash、失败计数和最长 7 天期限；明文只在 confirm 响应出现；
- status query 不需要已吊销的普通 session，但只能返回绑定 task 的 `DataTaskView`，错误 token/task、到期和成功后重放均拒绝。
...
对象 key 不写 audit/failure summary；对象服务使用 opaque ref 与 scope token。分享 URL 先失效、对象按 72 小时清理；C-014 v1 导出不创建对象或正文副本，仅有 24 小时 manifest。用户本地副本不在服务端删除范围。
...
- 导出按白名单投影解密并在内存中稳定序列化；不持久化正文，不导出 internal seed/ref、provider attempt、Safety 受限对象、audit、guard、status token 或删除回执 token；
...
- DataTask 展示删除的 guarded/online erased/backup deadline 与导出的 PREPARING/READY/EXPIRED/INVALIDATED 真实层次；
