...
- **当前任务**：[D-004 高保真原型、验证与开发交付](./current.md)（In Review；D-003 已接受并随 PR #144 合并）
- **路线图**：[ROADMAP.md](../ROADMAP.md)

## 1. Backlog 规则

- 本文件保存有序候选任务，不表示所有任务已经承诺；
- 同一时间只有 tasks/current.md 中一个任务可以是 Ready、In Progress 或 In Review；
- 任务进入执行前必须确认依赖和上游文档；
- 每个任务应能在一个主要 PR 中验收；
- 发现任务过大时先拆分，再开始编码；
...
| [E-016](https://github.com/WeiHan1996/DailyEnergy/issues/148) | 公开仓库与平台强制 CI Gate                 | In Progress | PUBLIC、main ruleset、11 required checks 与安全公开审计 |
