...
| `pnpm queue:test`                                    | 固定 digest 的真实 PostgreSQL 18 / Redis 8 + BullMQ 5，以及 AI-002 breaker CAS/loss |
...
- `registry/e010-evidence-manifest.json`、`registry/e011-evidence-manifest.json`、
  ...
  `registry/ai004-evidence-manifest.json`、`registry/ai005-evidence-manifest.json` 与已有
...
- AI-003 只登记 Prompt bytes/输入/版本 fail-closed 与 client/API/restricted Prompt 资产边界；
  真实 provider、AI-006 template 和未覆盖的文案质量结果仍保持 `PLANNED`；
- AI-004 只登记 deterministic candidate 的单 JSON/Schema/事实/ref/字符/人格/隐私/Safety、
  内容无关 receipt、完整 fallback 与 live guard 证据；
- AI-005 登记一个不可变单一人格参数策略、六份合成 Daily/Weekly 同事实样例、禁用语言、
  care/uncertainty 幽默上限和 profile 偏好未来生效/历史冻结证据；`E16-T02/T03` 的 120-output
  人工盲评、AI-014 runner、MODEL/LOAD/HUMAN 和真实 provider 继续保持 `PLANNED`；
...
- `queue/integration.test.mjs`：使用固定 digest 的真实 Redis 8.2.1、BullMQ 5.81.3 与
  ...
  重建、graceful drain，以及 breaker revision + route fingerprint CAS 与 Redis loss；
